...
def parse_shop_from_preview_element(element_html):
    ...
    if opening_hours_li_element:
        ...
        if span_next_to_icon:
            ...
            icon_text = opening_hours_li_element.select_one('i.mi.access_time').text.strip() if opening_hours_li_element.select_one('i.mi.access_time') else ''
...
async def fetch_shops_python_async():
    ...
    try:
        ...
        response = requests.get(full_url, headers={'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/98.0.4758.102 Safari/537.36'})
        ...
        if html_from_api:
            ...
            shop_elements = soup.select('div.lf-item-container')
            print(f"[fetch_shops_python_async] Parsed {len(shop_elements)} preview shops. Now fetching details concurrently...")
            ...
            async with aiohttp.ClientSession() as session:
                ...
                for shop_data in preview_shops:
                    if shop_data.get('details_url'):
                        ...
                        tasks.append(asyncio.sleep(0, result=None)) # Add a dummy completed future for shops with no URL
            ...
            for i, shop_data in enumerate(preview_shops):
                ...
                print(f"[fetch_shops_python_async] Final shop data: Name: {shop_data['name']}, Delivery: {shop_data['delivery_day']}")
        else:
